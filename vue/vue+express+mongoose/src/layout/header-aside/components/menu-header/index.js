...
export default {
    ...
    return createElement('div', {
      ...
      createElement('div', {
        ...
        createElement('div', {
          ...
          }, this.header.map(menu => (menu.children === undefined ? elMenuItem : elSubmenu).call(this, createElement, this.formatMenu(defaultsDeep({}, menu)))))
        ])
      ]),
      ...this.isScroll ? [
        createElement('div', {
  ...
  methods: {
    formatMenu(menu) {
      menu.title = this.$t(`navigator.${menu.title || 'unnameMenu'}`);
      if(menu.children) {
        menu.children.map(v => this.formatMenu(v));
      }
      return menu;
    },
    scroll (direction) {
